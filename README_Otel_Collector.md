...
1.  **gcloud 프로젝트 설정:**
    ...
    gcloud config set project p-khm8-dev-svc
...
[GKE에 Collector 배포][https://cloud.google.com/stackdriver/docs/instrumentation/opentelemetry-collector-gke?hl=ko]  참조
...
3.  **Collector Config 수정:**

    OpenTelemetry 라이브러리가 자동으로 생성하는 메트릭이 Google Cloud의 관리형 Prometheus 수집기에서 요구하는 데이터 타입과 달라서 타입 충돌이 발생합니다. 해당 에러 수정을 위해 Opentelemetry Collecotor의 default config를 수정해서 재배포해야 합니다. 
...
4.  **Google Cloud의 기존 측정항목 정의 삭제:**

    Google Cloud의 측정항목 정의(Metric Descriptor)는 한번 생성되면 상태를 유지(Statful)합니다. 혹시 기존에 생성된 메트릭 정의를 사용할 수 있기 때문에 에러가 발생하는 항목은 삭제하여 재생성하도록 API를 실행합니다. (관련 gcloud CLI는 부재합니다.)

    1) **app_messages_processed_count_total/counter 삭제:**
    ```bash
    curl -X DELETE \
    -H "Authorization: Bearer $(gcloud auth print-access-token)" \
    "https://monitoring.googleapis.com/v3/projects/$PROJECT_ID/metricDescriptors/prometheus.googleapis.com/app_messages_processed_count_total/counter"
    ```

    2) **otel_scope_info/gauge 삭제:**
    ```bash
    curl -X DELETE \
    -H "Authorization: Bearer $(gcloud auth print-access-token)" \
    "https://monitoring.googleapis.com/v3/projects/$PROJECT_ID/metricDescriptors/prometheus.googleapis.com/otel_scope_info/gauge"
    ```

---

### **Phase 5: Custom Metrics Stackdriver Adapter 설치**
